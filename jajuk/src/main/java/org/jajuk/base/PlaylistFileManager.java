...
public class PlaylistFileManager extends ItemManager implements Observer {
	...
	private PlaylistFileManager() {
		...
		// Hashcode
		registerProperty(new PropertyMetaInformation(XML_HASHCODE, false,
	 ...
	 * @param sName
	 */
	public synchronized PlaylistFile registerPlaylistFile(String sId,
			String sName, Directory dParentDirectory) throws Exception {
		synchronized (PlaylistFileManager.getInstance().getLock()) {
	 ...
	 */
	public PlaylistFile changePlaylistFileName(PlaylistFile plfOld,
			String sNewName) throws JajukException {
		synchronized (PlaylistFileManager.getInstance().getLock()) {
			...
			if (plfNew.getName().lastIndexOf('.') != plfNew.getName().indexOf(
					'.')// just one '.'
