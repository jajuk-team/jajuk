 ...
 *
 */
public class AlbumManager extends ItemManager<Album> implements Observer {

  /** Album max rating */
  private long maxRate = 0l;

  private TrackManager  trackManager  = (TrackManager) ItemType.Track.getManager();
   ...
   * @param sName
   */
  public Album registerAlbum(final String sName) {
   ...
   * @param sName
   */
  public Album registerAlbum(final String sId, final String sName) {
    synchronized (getLock()) {
      Album album = getAlbumByID(sId);

      if (album == null) {
        album = new Album(sId, sName);
        getItems().put(sId, album);
      }
      return album;
    }
  }
   ...
   * @return new album
   */
  public Album changeAlbumName(final Album old, final String sNewName) throws JajukException {
    // check there is actually a change
    if (old.getName2().equals(sNewName)) {
      return old;
    }
    final Album newItem = registerAlbum(sNewName);
    // re apply old properties from old item
    newItem.cloneProperties(old);
    // update tracks
    for (final Track track : trackManager.getCarbonItems()) {
      if (track.getAlbum().equals(old)) {
        trackManager.changeTrackAlbum(track, sNewName, null);
      }
    }
   ...
   * @return
   */
  public static synchronized String format(final String sName) {
    String sOut;
  ...
  /* (non-Javadoc)
   * @see org.jajuk.base.managers.ItemManager#getLabel()
   */
  @Override
   ...
   * @return Element
   */
  public Album getAlbumByID(final String sID) {
    synchronized (getLock()) {
   ...
   * @return
   */
  public Set<Album> getAssociatedAlbums(final Item item) {
    synchronized (getLock()) {
      final Set<Album> out = new TreeSet<Album>();
   ...
   *         available albums
   */
  public List<Album> getBestOfAlbums(final boolean bHideUnmounted, final int iNbBestofAlbums) {
    ...
    final HashMap<Album, Float> cacheRate = new HashMap<Album, Float>(getElementCount());
    // This hashmap contains album-> nb of tracks already taken into account
    // for average
    final HashMap<Album, Integer> cacheNb = new HashMap<Album, Integer>(getElementCount());
    for (final Track track : ((TrackManager) ItemType.Track.getManager()).getCarbonItems()) {
      if (track.getPlayeableFile(bHideUnmounted) != null) {
        ...
        final Float previousRate = cacheRate.get(track.getAlbum());
    ...
    final ArrayList<Album> sortedAlbums = new ArrayList<Album>(cacheRate.keySet());
   ...
   * @return newest albums
   */
  public List<Album> getNewestAlbums(final boolean bHideUnmounted, final int iNb) {
    // create a temporary table to remove unmounted albums
    // We consider an album as mounted if a least one track is mounted
    // This hashmap contains album-> discovery date
    final HashMap<Album, Date> cache = new HashMap<Album, Date>(getElementCount());
    for (final Track track : ((TrackManager) ItemType.Track.getManager()).getCarbonItems()) {
      if (track.getPlayeableFile(bHideUnmounted) != null) {
    ...
    final ArrayList<Album> sortedAlbums = new ArrayList<Album>(cache.keySet());
    Collections.sort(sortedAlbums, new Comparator<Album>() {
      public int compare(final Album o1, final Album o2) {
   ...
   *         available albums
   */
  public List<Album> getRarelyListenAlbums(final boolean bHideUnmounted, final int iNb) {
    // create a temporary table to remove unmounted albums
    // We consider an album as mounted if a least one track is mounted
    // This hashmap contains album-> album hits (each track hit average)
    final HashMap<Album, Float> cache = new HashMap<Album, Float>(getElementCount());
    // This hashmap contains album-> nb of tracks already taken into account
    // for average
    final HashMap<Album, Integer> cacheNb = new HashMap<Album, Integer>(getElementCount());
    for (final Track track : ((TrackManager) ItemType.Track.getManager()).getCarbonItems()) {
      if (track.getPlayeableFile(bHideUnmounted) != null) {
        ...
        final Float previousRate = cache.get(track.getAlbum());
        if (previousRate == null) {
   ...
   *         available albums
   */
  private List<Album> getTopAlbums(final List<Album> sortedAlbums, final int iNb) {
    ...
    final List<Album> sublist = sortedAlbums.subList(sortedAlbums.size() - (1 + size), sortedAlbums
        .size() - 1);
    // Shuffle the result
    Collections.shuffle(sublist);
    // The result is a sublist of shuffled albums, if we have less
    // albums than required, take max size possible
    final List<Album> out = sublist.subList(0, (size >= iNb) ? iNb : size);
    return out;
  }
  ...
  public void refreshMaxRating() {
    ...
    final HashMap<Album, Integer> cacheNb = new HashMap<Album, Integer>(getElementCount());
    for (final Track track : ((TrackManager) ItemType.Track.getManager()).getCarbonItems()) {
      if (track.getPlayeableFile(ConfigurationManager.getBoolean(ConfKeys.OPTIONS_HIDE_UNMOUNTED)) != null) {
        float newRate = 0f;
        Integer nb = cacheNb.get(track.getAlbum());
        if (nb == null) {
          nb = 0;
    ...
    for (final Album album : cacheRate.keySet()) {
      final long value = Math.round(cacheRate.get(album));
      if (value > maxRate) {
        maxRate = value;
      }
    }
  ...
  public Album getAlbumByName(final String name) {
    ...
    for (final Album album : getCarbonItems()) {
