...
public class HTMLExporter extends Exporter implements ITechnicalStrings {
  ...
  public void processCollection(int type) throws Exception {
    ...
    if (type == PHYSICAL_COLLECTION) {
      ...
      XMLTransformer.xmlToHTML(xmlExporter.getCacheFile(), cache, new File(XSLT_DEVICE.getFile()));
      ...
      XMLTransformer.xmlToHTML(xmlExporter.getCacheFile(), cache, new File(XSLT_COLLECTION_LOGICAL
          .getFile()));
    }
  }
  ...
  public void process(ArrayList<Item> collection) throws Exception {
    ...
    if (first instanceof Style) {
      XMLTransformer.xmlToHTML(xmlExporter.getCacheFile(), cache, new File(XSLT_STYLE.getFile()));
    } else if (first instanceof Author) {
      XMLTransformer.xmlToHTML(xmlExporter.getCacheFile(), cache, new File(XSLT_AUTHOR.getFile()));
    } else if (first instanceof Year) {
      XMLTransformer.xmlToHTML(xmlExporter.getCacheFile(), cache, new File(XSLT_YEAR.getFile()));
    } else if (first instanceof Album) {
      XMLTransformer.xmlToHTML(xmlExporter.getCacheFile(), cache, new File(XSLT_ALBUM.getFile()));
    } else if (first instanceof Device) {
      XMLTransformer.xmlToHTML(xmlExporter.getCacheFile(), cache, new File(XSLT_DEVICE.getFile()));
    } else if (first instanceof Directory) {
      XMLTransformer.xmlToHTML(xmlExporter.getCacheFile(), cache,
          new File(XSLT_DIRECTORY.getFile()));
    }
  }
