...
public class LyrcProvider extends GenericProvider {
  ...
  /* (non-Javadoc)
   * @see ext.services.lyrics.providers.GenericProvider#getLyrics(java.lang.String, java.lang.String)
   */
  @Override
  public String   getLyrics(final String artist, final String title) {
    String        html  = super.getLyrics(artist, title);
    
    if (html.contains("Suggestions : <br>")) {
      ...
      while (html.indexOf("href=\"") != -1) {
        String uri  = html.substring(html.indexOf("href=\"") + 6); // from html tag <a href="....
        String text = html.substring(html.indexOf("'white'>") + 8);// from font color='white'>...
      ...
      for (final String suggestion : suggestions.keySet()) {
        ...
        if (matches) {
          ...
          try {
            return (cleanLyrics(DownloadManager.downloadHtml(new URL(suggestionURL), "ISO-8859-1")));
          }
   ...
   * The correct subsection is to be extracted first,
   * before being cleaned and stripped from useless HTML tags.
   * 
   * @return  the lyrics
   */
