...
public class AmbienceWizard extends Wizard implements ITechnicalStrings {
	...
	public static class AmbiencePanel extends Screen implements ActionListener {
		...
		public void actionPerformed(final ActionEvent ae) {
			if (ae.getSource() == jbNew) {
				AmbienceWizard.ambiences.add(new Ambience(Long.toString(System
				...
				AmbienceWizard.ambiences = new ArrayList<Ambience>(AmbienceManager
			...
			ObservationManager.notify(new Event(
					EventSubject.EVENT_AMBIENCES_CHANGE));
		}
			...
			for (int index = 0; index < AmbienceWizard.ambiences.size(); index++) {
				...
				jtfName.addCaretListener(new CaretListener() {
					public void caretUpdate(final CaretEvent arg0) {
						final int index = AmbienceWizard.getWidgetIndex(widgets, (JComponent) arg0
								.getSource());
						...
						for (int i = 0; i < widgets.length; i++) {
							...
							if (jtf.getText().equals(s)) {
								setProblem(Messages
										.getString("DigitalDJWizard.60"));
								return;
							}
						}
						// reset previous problems
						if ((s.length() == 0)
								|| (((JButton) widgets[index][2]).getText()
										.length() == 0)) {
							setProblem(Messages.getString("DigitalDJWizard.39"));
						} else {
							setProblem(null);
				...
				jrbAmbience.addActionListener(new ActionListener() {
					public void actionPerformed(final ActionEvent ae) {
						((JTextField) widgets[AmbienceWizard.getWidgetIndex(widgets,
								jrbAmbience)][1]).getText();
				...
				jbStyle.addActionListener(new ActionListener() {
					public void actionPerformed(final ActionEvent ae) {
						final int row = AmbienceWizard.getWidgetIndex(widgets, (JComponent) ae
								.getSource());
						addStyle(row);
    ...
    public void initUI() {
			...
			jbNew = new JButton(
					Messages.getString("DigitalDJWizard.32"), IconLoader.ICON_NEW);
			jbNew.addActionListener(this);
			jbNew.setToolTipText(Messages.getString("DigitalDJWizard.33"));
			jbDelete = new JButton(
					Messages.getString("DigitalDJWizard.34"), IconLoader.ICON_DELETE);
			jbDelete.addActionListener(this);
			jbDelete.setToolTipText(Messages.getString("DigitalDJWizard.35"));
			jbDefaults = new JButton(
					Messages.getString("DigitalDJWizard.62"), IconLoader.ICON_DEFAULTS);
	...
	public AmbienceWizard() {
		super(Messages.getString("DigitalDJWizard.56"), AmbiencePanel.class,
				Util.getImage(ITechnicalStrings.IMAGE_DJ), Main.getWindow(), new Locale(Messages
