        ...
        for (int i = 0; i < files.length; i++) {
            try{ //check errors for each file
                ...
                if (!new File(files[i].getAbsolutePath()).exists()){
                    Log.warn("Cannot read file name (please rename it): {{"+files[i].getAbsolutePath()+"}}"); //$NON-NLS-1$ //$NON-NLS-2$
                    continue;
                }
                ...
                if (bIsMusic) {
                    ...
                    if (tag.isCorrupted()){
                        ...
                        Log.error("103","{{"+files[i].getAbsolutePath()+"}}",null); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
                    }
                ...
                Log.error("103",files.length>0?"{{"+files[i].toString()+"}}":"",e); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
            }
        }
    }
