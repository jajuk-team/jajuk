...
public class TrackManager extends ItemManager implements Observer {
    ...
    private TrackManager() {
        ...
        registerProperty(new PropertyMetaInformation(XML_TRACK_TYPE, false, true, true, false,
                false, Long.class, null));
        // Year
        registerProperty(new PropertyMetaInformation(XML_TRACK_YEAR, false, true, true, true, true,
                Long.class, 0));
        // Rate
    ...
    public Track changeTrackAlbum(Track track, String sNewAlbum, HashSet filter)
        ...
        synchronized (TrackManager.getInstance().getLock()) {
            ...
            AlbumManager.getInstance().cleanup(track.getAlbum()); // remove this album if no more
                                                                    // references
            return newTrack;
        }
    }
        ...
        synchronized (TrackManager.getInstance().getLock()) {
            ...
            Track newTrack = registerTrack(track.getName(), track.getAlbum(), track.getStyle(),
                    newAuthor, track.getLength(), track.getYear(), track.getOrder(), track
                            .getType());
            postChange(track, newTrack, filter);
            AuthorManager.getInstance().cleanup(track.getAuthor()); // remove this item if no more
                                                                    // references
            return newTrack;
        }
    }
    ...
    public Track changeTrackStyle(Track track, String sNewStyle, HashSet filter)
        ...
        synchronized (TrackManager.getInstance().getLock()) {
            ...
            Track newTrack = registerTrack(track.getName(), track.getAlbum(), newStyle, track
                    ...
                    .getType());
            postChange(track, newTrack, filter);
            StyleManager.getInstance().cleanup(track.getStyle()); // remove this item if no more
                                                                    // references
            return newTrack;
        }
    }
    ...
    public Track changeTrackYear(Track track, long lNewItem, HashSet filter) throws JajukException {
        synchronized (TrackManager.getInstance().getLock()) {
            ...
            // Register new item
            Track newTrack = registerTrack(track.getName(), track.getAlbum(), track.getStyle(),
                    track.getAuthor(), track.getLength(), lNewItem, track.getOrder(), track
                            .getType());
            postChange(track, newTrack, filter);
            return newTrack;
        }
    ...
    private void postChange(Track track, Track newTrack, HashSet filter) {
        synchronized (TrackManager.getInstance().getLock()) {
            ...
            if (track.getFiles().size() == 0) { // normal case: old track has no more associated
                                                // tracks, remove it
