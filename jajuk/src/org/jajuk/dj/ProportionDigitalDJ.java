...
public class ProportionDigitalDJ extends DigitalDJ implements ITechnicalStrings {
	...
	private List<File> getSequence() {
		List<File> out = new ArrayList<File>(100);
		HashMap<Proportion, List<File>> list = new HashMap<Proportion, List<File>>(
				10);
		// get a global shuffle selection, we will keep only tracks with wanted
		// styles
		List<File> global = FileManager.getInstance()
				.getGlobalShufflePlaylist();
		...
		for (Proportion prop : list.keySet()) {
			List<File> files = list.get(prop);
			out.addAll(files
					.subList(0, (int) (iMinSize * prop.getProportion())));
		}
	...
	public String toXML() {
		...
		for (Proportion proportion : proportions) {
			...
			sb
					.append("\t\t<" + XML_DJ_PROPORTION + " " + XML_DJ_STYLES + "='" + stylesDesc + "' " + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
							XML_DJ_VALUE
