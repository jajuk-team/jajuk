...
public class Messages extends DefaultHandler implements ITechnicalStrings	{
    ...
    public static void showWarningMessage(String sMessage){
        DetailsMessageDialog message = new DetailsMessageDialog(sMessage,getTitleForType(JOptionPane.WARNING_MESSAGE),JOptionPane.WARNING_MESSAGE,null,null);
        if (SwingUtilities.isEventDispatchThread()){ //in the dispatcher thread, no need to use invokeLatter
            message.run();
        }
    ...
    public static void showHideableWarningMessage(String sMessage,String sProperty){
        HideableMessageDialog message = new HideableMessageDialog(
            sMessage,
    ...
    public static void showInfoMessage(final String sMessage,final Icon icon){
        DetailsMessageDialog message = new DetailsMessageDialog(sMessage,getTitleForType(JOptionPane.INFORMATION_MESSAGE),JOptionPane.INFORMATION_MESSAGE,null,icon);
        if (SwingUtilities.isEventDispatchThread()){ //in the dispatcher thread, no need to use invokeLatter
            message.run();
        }
        else{ //not in the awt dispatcher thread
    ...
    public static void showErrorMessage(final String sCode,final String sInfoSup){
        DetailsMessageDialog message = new DetailsMessageDialog(Messages.getErrorMessage(sCode)+" : "+sInfoSup,getTitleForType(JOptionPane.ERROR_MESSAGE),JOptionPane.ERROR_MESSAGE,null,null); //$NON-NLS-1$
        if (SwingUtilities.isEventDispatchThread()){ //in the dispatcher thread, no need to use invokeLatter
            message.run();
        }
        else{ //not in the awt dispatcher thread
    ...
    public static void showDetailedErrorMessage(final String sCode,final String sInfoSup,String sDetails){
        DetailsMessageDialog message = new DetailsMessageDialog(Messages.getErrorMessage(sCode)+" : "+sInfoSup,getTitleForType(JOptionPane.ERROR_MESSAGE),JOptionPane.ERROR_MESSAGE,sDetails,null); //$NON-NLS-1$
        if (SwingUtilities.isEventDispatchThread()){ //in the dispatcher thread, no need to use invokeLatter
            message.run();
        }
        else{ //not in the awt dispatcher thread
    ...
    public static void showInfoMessage(final String sMessage,final String sInfoSup){
        DetailsMessageDialog message = new DetailsMessageDialog(sMessage+" : "+sInfoSup,getTitleForType(JOptionPane.INFORMATION_MESSAGE),JOptionPane.INFORMATION_MESSAGE,null,null); //$NON-NLS-1$
        if (SwingUtilities.isEventDispatchThread()){ //in the dispatcher thread, no need to use invokeLatter
            message.run();
        }
        else{ //not in the awt dispatcher thread
    ...
    public static void showInfoMessage(final String sMessage){
        DetailsMessageDialog message = new DetailsMessageDialog(sMessage,getTitleForType(JOptionPane.INFORMATION_MESSAGE),JOptionPane.INFORMATION_MESSAGE,null,null);
        if (SwingUtilities.isEventDispatchThread()){ //in the dispatcher thread, no need to use invokeLatter
            message.run();
        }
        else{ //not in the awt dispatcher thread
...
class HideableMessageDialog implements Runnable,ITechnicalStrings{
    ...
    public void run() {
        JOptionPane optionPane = Util.getNarrowOptionPane(72);
        optionPane.setMessage(sText);
        Object[] options = { Messages.getString("OK"), Messages.getString("Hide") }; //$NON-NLS-1$ //$NON-NLS-2$
        optionPane.setOptions(options);
        optionPane.setMessageType(iType);
        if (icon != null){
            optionPane.setIcon(icon);
        ...
        JDialog dialog = optionPane.createDialog(null,sTitle);
        dialog.setVisible(true);
        if (optionPane.getValue().equals(Messages.getString("Hide"))){ //Not shoow again
