...
public class JavaLayerPlayerImpl implements IPlayerImpl,ITechnicalStrings,BasicPlayerListener{
	...
	private long lTime = 0; //must be static to be correctly accessed by the player listener inner class
	...
	public synchronized void play(org.jajuk.base.File file,float fPosition,long length,float fVolume) throws Exception{
		...
		if (player == null){
			...
			BasicPlayer.EXTERNAL_BUFFER_SIZE = 512000;
	...
	public void seek(float posValue){
		...
		if (mPlayingData.containsKey("audio.type") && player!=null) { //$NON-NLS-1$
			...
			if (Boolean.valueOf(type.getProperty(TYPE_PROPERTY_SEEK_SUPPORTED)).booleanValue() 
				...
				long skipBytes =(long) Math.round(((Integer)mPlayingData.get("audio.length.bytes")).intValue()* posValue); //$NON-NLS-1$
				try {
