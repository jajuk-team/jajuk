...
public class JajukListener implements ActionListener, ITechnicalStrings {
	...
	public void actionPerformed(final ActionEvent e) {
		try {
			// no thread, nothing requires long execution time and a
			// SwingWorker is not adapted
			if (e.getActionCommand().equals(
					EventSubject.EVENT_REPEAT_MODE_STATUS_CHANGED.toString())) {
				...
				if (!b == true) { // enabled button
					CommandJPanel.getInstance().jbRandom
							.setBorder(BorderFactory.createLoweredBevelBorder());
					FIFO.getInstance().shuffle(); // shuffle current
					// selection
					// now make sure we can't have a single repeated file
					// after a non-repeated file (by design)
				...
				FIFO.getInstance().computesPlanned(true);
					...
					EventSubject.EVENT_CONTINUE_MODE_STATUS_CHANGED.toString())) {
				boolean b = ConfigurationManager
				...
				if (!b == true) { // enabled button
					CommandJPanel.getInstance().jbContinue
							.setBorder(BorderFactory.createLoweredBevelBorder());
					if (FIFO.isStopped()) {
				...
				FIFO.getInstance().computesPlanned(false);
					...
					EventSubject.EVENT_INTRO_MODE_STATUS_CHANGED.toString())) {
				boolean b = Boolean.valueOf(
						ConfigurationManager.getProperty(CONF_STATE_INTRO))
						.booleanValue();
				ConfigurationManager.setProperty(CONF_STATE_INTRO, Boolean
						.toString(!b));
				JajukJMenuBar.getInstance().jcbmiIntro.setSelected(!b);
				if (!b == true) { // enabled button
					CommandJPanel.getInstance().jbIntro.setBorder(BorderFactory
					...
					EventSubject.EVENT_VIEW_RESTORE_DEFAULTS.toString())) {
			...
			} else if (EventSubject.EVENT_HELP_REQUIRED.equals(e
					.getActionCommand())) {
				...
				fsw.setVisible(true);
			} else if (EventSubject.EVENT_CREATE_PROPERTY.equals(e
					.getActionCommand())) {
				NewPropertyWizard npw = new NewPropertyWizard();
				npw.pack();
				npw.setLocationRelativeTo(Main.getWindow());
				npw.setVisible(true);
			} else if (EventSubject.EVENT_DELETE_PROPERTY.equals(e
					.getActionCommand())) {
				...
				qfw.setVisible(true);
			} else if (EventSubject.EVENT_TIP_OF_THE_DAY.equals(e
					.getActionCommand())) {
