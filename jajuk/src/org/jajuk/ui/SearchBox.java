...
public class SearchBox extends JTextField implements KeyListener {
    ...
    /**
         * Perform a search when user stop to type in the search combo for 2 sec
         * or pressed enter
         */
    private void search() {
	try {
	    ...
	    if (sTyped.length() >= MIN_CRITERIA_LENGTH) { // second test
                                                                // to get sure
                                                                // user didn't
                                                                // typed before
                                                                // entering this
                                                                // method
		TreeSet tsResu = FileManager.getInstance().search(
			sTyped.toString());
		if (tsResu.size() > 0) {
