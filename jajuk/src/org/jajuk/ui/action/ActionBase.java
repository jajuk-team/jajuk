...
/**
 * Common super class for Swing actions. This class provides useful construction options to create actions, just leaving open the necessity of implementing the {@link #actionPerformed(java.awt.event.ActionEvent)} method.
 * 
 * @author Bart Cremers
 * @since 12-dec-2005
 */
public abstract class ActionBase extends AbstractAction implements ITechnicalStrings,
    ...
    /**
     * Jintellitype object used for hotkeys and intellitype events management under winodws only
     */
     ...
     * 
     * @param name
     *            The unique name for the action. This name is used in the labels for visualization. If the name contains a '_' (underscore) character. The character following this underscore is used as mnemonic key for the action.
     * @param icon
     ...
     */
    protected ActionBase(String pName, Icon icon, KeyStroke stroke, boolean enabled) {
        String name = pName;
        if (name != null) {
     ...
     *            The icon to use for visualization of the action.
     * @param stroke
     *            The keystroke to use. If the keystroke given is not a valid keystroke using the rules describe in {@link javax.swing.KeyStroke#getKeyStroke(String)}, null is used instead.
     * @param enabled
     *            By default enable or disable the action.
     */
    ...
    /**
     * @param description
     *            The short description for the action. This is used for tooltip text.
     */
    ...
    /**
     * @param description
     *            The long description for the action. This can be used for context-sensitive help.
     */
    ...
    /**
     * @param actionCommand
     *            The action command for this action. This is used for creating the <code>ActionEvent</code>.
     */
    ...
    /**
     * @param stroke
     *            The keystroke for the action. If the keystroke given is not a valid keystroke using the rules described in {@link javax.swing.KeyStroke#getKeyStroke(String)}, <code>null</code> is used instead. This is used as a shortcut key.
     */
    ...
    /**
     * Sets the keyboard mnemonic on the current action. <p/> A mnemonic must correspond to a single key on the keyboard and should be specified using one of the <code>VK_XXX</code> keycodes defined in <code>java.awt.event.KeyEvent</code>. Mnemonics are case-insensitive, therefore a key event with the corresponding keycode would cause the button to be activated whether or not the Shift
     * modifier was pressed. <p/> If the character defined by the mnemonic is found within the button's label string, the first occurrence of it will be underlined to indicate the mnemonic to the user.
     * 
     * @param mnemonic
     *            The key code which represents the mnemonic. The mnemonic is the key which when combined with the look and feel's mouseless modifier (usually <b>Alt</b>) will activate this button if focus is contained somewhere within this action's ancestor window.
    ...
    /**
     * Invoked when an action occurs. This implementation calls {@link #perform(java.awt.event.ActionEvent)} to add error handling and logging to the action system.
     * 
     * @param evt
     *            The event.
     */
