 ...
 * @created 29 dec. 2003
 */
public abstract class AbstractPlaylistEditorView extends ViewAdapter implements
	Observer, MouseListener, ActionListener, ListSelectionListener,
	...
	final Font fontPlanned = new Font("serif", Font.ITALIC, 12); // font
                                                                        // for
                                                                        // planned
                                                                        // items
                                                                        // //$NON-NLS-1$
	...
	public synchronized void prepareColumns() {
	    ...
	    // Album
	    vColNames.add(Messages.getString(PROPERTY_SEPARATOR
		    + XML_TRACK_ALBUM)); //$NON-NLS-1$
	    vId.add(XML_TRACK_ALBUM); //$NON-NLS-1$

	    // Author
	    vColNames.add(Messages.getString(PROPERTY_SEPARATOR
		    + XML_TRACK_AUTHOR)); //$NON-NLS-1$
	    vId.add(XML_TRACK_AUTHOR); //$NON-NLS-1$

	    // Style
	    vColNames.add(Messages.getString(PROPERTY_SEPARATOR
		    + XML_TRACK_STYLE)); //$NON-NLS-1$
	    vId.add(XML_TRACK_STYLE); //$NON-NLS-1$

	    // Stars
	    vColNames.add(Messages.getString(PROPERTY_SEPARATOR
		    + XML_TRACK_RATE)); //$NON-NLS-1$
	    vId.add(XML_TRACK_RATE); //$NON-NLS-1$

	    // Year
	    vColNames.add(Messages.getString(PROPERTY_SEPARATOR
		    + XML_TRACK_YEAR)); //$NON-NLS-1$
	    ...
	    vColNames.add(Messages
		    .getString(PROPERTY_SEPARATOR + XML_DIRECTORY));
	...
	/**
         ...
         */
	public synchronized void populateModel(String sPropertyName,
		String sPattern) {
		...
		if (plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_QUEUE) {
			...
			if (item.isRepeat()) {
			    oValues[iRow][0] = new IconLabel(
				    iconRepeat,
				    "", null, null, font, Messages.getString("AbstractPlaylistEditorView.19")); // normal
                                                                                                                // file,
                                                                                                                // repeated
                                                                                                                // //$NON-NLS-1$
                                                                                                                // //$NON-NLS-2$
			    ...
			    oValues[iRow][0] = new IconLabel(
				    iconNormal,
				    "", null, null, font, Messages.getString("AbstractPlaylistEditorView.18")); // normal
                                                                                                                // file,
                                                                                                                // not
                                                                                                                // repeated
                                                                                                                // //$NON-NLS-1$
		    ...
		    oValues[iRow][0] = new IconLabel(
			    iconPlaylist,
			    "", null, null, font, Messages.getString("AbstractPlaylistEditorView.21")); //$NON-NLS-1$ //$NON-NLS-2$
		}
		// Track name
		...
		// Length
		oValues[iRow][7] = Util.formatTimeBySec(bf.getTrack()
			.getLength(), false);
		// Comment
		oValues[iRow][8] = bf.getTrack().getStringValue(
			XML_TRACK_COMMENT);
		// Date discovery
		oValues[iRow][9] = bf.getTrack().getAdditionDate(); // show
                                                                        // date
                                                                        // using
                                                                        // default
                                                                        // local
		...
		// Custom properties now
		// for tracks
		Iterator it2 = TrackManager.getInstance().getCustomProperties()
			.iterator();
		...
		// for files
		it2 = FileManager.getInstance().getCustomProperties()
			.iterator();
		// note that index lust start at custom track properties size
		for (int i = TrackManager.getInstance().getCustomProperties()
			.size(); it2.hasNext(); i++) {
		    PropertyMetaInformation meta = (PropertyMetaInformation) it2
			    .next();
	...
	jtable
		.setSelectionMode(DefaultListSelectionModel.MULTIPLE_INTERVAL_SELECTION); // multi-row
                                                                                                // selection
	jtable.setSortable(false);
	jtable.setDragEnabled(true);
	jtable.setTransferHandler(new PlaylistEditorTransferHandler(jtable));
	setRenderers();
	jtable.getColumnModel().getColumn(0).setPreferredWidth(20); // just
                                                                        // an
                                                                        // icon
	...
	jtable.addHighlighter(new ConditionalHighlighter(Color.ORANGE,
		...
		if (itemCurrent != null && itemCurrent.equals(item)) { // if it
                                                                        // is
                                                                        // the
                                                                        // currently
                                                                        // played
                                                                        ...
                                                                        // change
                                                                        // color
		    if (plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_QUEUE) { // for
                                                                                        // queue
                                                                                        // playlist,
                                                                                        // only
                                                                                        // highlight
	...
	jtable.getColumnModel().addColumnModelListener(this); // add this
                                                                // listener
                                                                // after hiding
                                                                // columns
	...
	jmiFilePlay = new JMenuItem(Messages
		.getString("AbstractPlaylistEditorView.23")); //$NON-NLS-1$
	jmiFilePlay.addActionListener(this);
	jmiFilePush = new JMenuItem(Messages
		.getString("AbstractPlaylistEditorView.24")); //$NON-NLS-1$
	jmiFilePush.addActionListener(this);
	jmiFileAddFavorites = new JMenuItem(Messages
		.getString("AbstractPlaylistEditorView.25")); //$NON-NLS-1$
	jmiFileAddFavorites.addActionListener(this);
	jmiFileProperties = new JMenuItem(Messages
		.getString("AbstractPlaylistEditorView.26")); //$NON-NLS-1$
	...
	SwingUtilities.invokeLater(new Runnable() {
		...
		try {
		    ...
		    bReloading = true; // flag reloading to avoid wrong column
                                        // events
		    Object origin = ObservationManager.getDetail(event,
			    DETAIL_ORIGIN);
		    // changed of playlist
		    if (EventSubject.EVENT_PLAYLIST_SELECTION_CHANGED
			    .equals(subject)
			...
			// clear planned
			alPlanned = new ArrayList<StackItem>(0); // make
                                                                        // sure
                                                                        // planned
                                                                        // is
                                                                        // voided
                                                                        ...
                                                                        // in
                                                                        // Queue
			jtable.getSelectionModel().clearSelection(); // remove
                                                                        // selection
			PlaylistFileItem plfi = (PlaylistFileItem) ObservationManager
				.getDetail(event, DETAIL_SELECTION);
		    ...
		    else if (EventSubject.EVENT_PLAYLIST_REFRESH
			    .equals(subject)
			...
			try {
				...
				alItems = Util.createStackItems(plfi
					.getPlaylistFile().getFiles(),
					ConfigurationManager
						.getBoolean(CONF_STATE_REPEAT),
					true); // PERF
			    }
			    ((JajukTableModel) jtable.getModel())
				    .populateModel();
			...
			for (int i = 0; i < rows.length; i++) {
			    jtable.getSelectionModel().addSelectionInterval(
				    rows[i], rows[i]); // set saved selection
                                                        // after a refresh
			}
			...
			alItems.clear();
			alPlanned.clear();
			update(new Event(EventSubject.EVENT_PLAYLIST_REFRESH)); // refresh
                                                                                // playlist
                                                                                // editor
			...
			model = new PlayListEditorTableModel();// create a new
                                                                // model
			...
			jtable.removeColumnFromConf((String) properties
				.get(DETAIL_CONTENT));
			jtable.showColumns(jtable.getColumnsConf());
		    }
		} catch (Exception e) {
	...
	if (e.getClickCount() == 2) { // double clic, launches selected track
                                        // and all after
		...
		if (plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_QUEUE) {
			...
			item.setRepeat(ConfigurationManager
				.getBoolean(CONF_STATE_REPEAT));
		...
		// For others playlists, we launch all tracks from this position
		    ...
		    FIFO
			    .getInstance()
			    .push(
				    getItemsFrom(jtable.getSelectedRow()),
				    ConfigurationManager
	...
	if (index < alItems.size()) {
	    return new ArrayList<StackItem>(alItems.subList(index, alItems
		    .size()));
	} else {
	    return null;
	}
	...
	try {
		...
		if (plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_NORMAL) {
		    if (this instanceof LogicalPlaylistEditorView) { // if
                                                                        // logical
                                                                        // editor,
                                                                        // warning
                                                                        // message
			StringBuffer sbOut = new StringBuffer(Messages
				.getString("AbstractPlaylistEditorView.17")); //$NON-NLS-1$
			Playlist pl = PlaylistManager.getInstance()
				.getPlaylistByID(
			...
			if (pl != null) {
				...
				while (it.hasNext()) {
				    ...
				    try {
					plf.setFiles(plfi.getPlaylistFile()
						.getFiles()); // set same
                                                                // files for all
                                                                // playlist
                                                                // files
					...
					InformationJPanel
						...
						.setMessage(
							Messages
								.getString("AbstractPlaylistEditorView.22"), InformationJPanel.INFORMATIVE); //$NON-NLS-1$
			...
			try {
			    ...
			    Log.error(je);
			    Messages.showErrorMessage(je.getCode(), je
				    .getMessage());
			}
		    }
		} else { // special playlist, same behavior than a save
                                // as
		...
		if (plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_QUEUE) { // if
                                                                                // it
                                                                                // is
                                                                                // the
                                                                                // queue
		...
		if (iRows.length > 1) {// if multiple selection, remove
                                        // selection
		    jtable.getSelectionModel().removeIndexInterval(0,
			    jtable.getRowCount() - 1);
		}
		for (int i = 0; i < iRows.length; i++) {
		    plfi.getPlaylistFile().remove(iRows[i] - i); // don't
                                                                        // forget
                                                                        // that
                                                                        // index
                                                                        // changes
		...
		if (iRows[0] == jtable.getRowCount()) {
		    jtable.getSelectionModel().setSelectionInterval(iLastRow,
			    iLastRow);
		}
	    ...
	    } else if (ae.getSource() == jmiFilePlay
		    || (ae.getSource() == jmiFilePush)) {
		// computes selected items
		ArrayList<StackItem> alItemsToPlay = new ArrayList<StackItem>(
			jtable.getSelectedRowCount());
		int[] indexes = jtable.getSelectedRows();
		for (int i = 0; i < indexes.length; i++) {
		    alItemsToPlay.add(getItem(indexes[i]));
		}
		FIFO.getInstance().push(alItemsToPlay,
			ae.getSource() == jmiFilePush);
		...
		ArrayList<File> alFiles = new ArrayList<File>(alItemsToPlay
			.size());
    ...
    /**
         * Get the playlist file items in the associated playlist repository
         * view
         * 
         * @return
         */
	...
	if (e.getValueIsAdjusting() || bSettingSelection) { // leave during
                                                                // normal
                                                                // refresh
	    return;
	}
	...
	if (!lsm.isSelectionEmpty()) {
	    ...
	    boolean bPlanned = false; // true if selected line is a planned
                                        // track
	    if (selectedRow > alItems.size() - 1) { // means it is a planned
                                                        // track
		bPlanned = true;
	    }
		...
		if (plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_QUEUE
			...
			|| plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_NOVELTIES) { // neither
                                                                                                // for
                                                                                                // bestof
                                                                                                // or
                                                                                                // novelties
	    ...
	    // Add shuffle button
	    if (plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_BESTOF // neither
                                                                        // for
                                                                        // bestof
                                                                        // playlist
		    || plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_NOVELTIES
		    || lsm.getMinSelectionIndex() != lsm.getMaxSelectionIndex() // multiple
                                                                                // selection
                                                                                // not
                                                                                // supported
		    || (plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_QUEUE && lsm
			    .getMinSelectionIndex() == 0)// can't add
                                                                // track at
                                                                // current track
                                                                // position
		    || (plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_QUEUE && selectedRow > FIFO
			    .getInstance().getFIFO().size())// no add for
                                                                // planned track
                                                                // but user can
                                                                // add over
                                                                // first planned
	    ...
	    // Up button
	    if (lsm.getMinSelectionIndex() != lsm.getMaxSelectionIndex() // check
                                                                                // if
                                                                                // several
                                                                                // rows
                                                                                // have
                                                                                ...
                                                                                // supported
                                                                                // yet
		    || (plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_QUEUE && FIFO
			    .getInstance().containsRepeat()) // check if we
                                                                // are in the
                                                                // queue with
                                                                // repeated
                                                                // tracks : not
		...
		if (bPlanned) {
		    if (lsm.getMaxSelectionIndex() > alItems.size()) { // a
                                                                        // planned
                                                                        // track
                                                                        // can't
                                                                        // go to
		...
		if (bPlanned) {
		    if (lsm.getMaxSelectionIndex() < alItems.size()
			    + alPlanned.size() - 1) { // can't go further
		    ...
		    if (plfi.getType() == PlaylistFileItem.PLAYLIST_TYPE_QUEUE
			    && lsm.getMaxSelectionIndex() == 0) { // current
                                                                        // track
                                                                        // can't
                                                                        // go
                                                                        // down
			jbDown.setEnabled(false);
		    } else if (lsm.getMaxSelectionIndex() < alItems.size() - 1) { // a
                                                                                        // normal
                                                                                        // item
                                                                                        // can't
                                                                                        // go
