...
public class ViewManager implements ITechnicalStrings {
    ...
    /** Views */
    static ArrayList<IView> alViews = new ArrayList<IView>(20);

    /** containers hashmap */
    static ArrayList<JajukContainer> alContainers = new ArrayList<JajukContainer>(20);

    /** DockingViews */
    static ArrayList<View> alDockingViews = new ArrayList<View>(20);
    ...
    public static net.infonode.docking.View makeDockingView(final IView view) {
        final JajukContainer jc = new JajukContainer(view);
        final net.infonode.docking.View dockingView = new net.infonode.docking.View(Messages
                .getString(view.getDesc()), Util.getIcon(ICON_LOGO_FRAME), jc);
        return dockingView;
    }
     ...
     * @param view
     */
    public static void notify(EventSubject sEvent, final IView view) {
        try {
            if (sEvent.equals(EventSubject.EVENT_VIEW_REFRESH_REQUEST)) {
                if (!view.isPopulated()) {
                    view.initUI();
                }
                view.refresh();
            } else if (sEvent.equals(EventSubject.EVENT_VIEW_CLOSE_REQUEST)) {
                // setVisible(view,false);
            } else if (sEvent.equals(EventSubject.EVENT_VIEW_SHOW_REQUEST)) {
                addViewToPerspective(view);
            }
        } catch (Exception e) {
            Log.error("118", sEvent.toString(), e); //$NON-NLS-1$
            Messages.showErrorMessage("118", sEvent.toString()); //$NON-NLS-1$
        }
    }
     ...
     *            views class
     */
    public static void notify(EventSubject sEvent, Class c) {
