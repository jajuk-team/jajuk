...
public class MD5Processor {
	...
	public static final byte[] hash(byte[] bIn) {
		...
		int n = b1.length / 4; //n=nb 32 bits-word we have in the padded message
		int[] x;
		int aa = a;
		int bb = b;
		int cc = c;
		int dd = d;
		for (int i = 0; i < (n / 16); i++) { //for each packet of 512 bits ( 64 bytes )
			x = new int[16];
			for (int j = 0; j < 16; j++) {
				x[j] =
	 ...
	 */
	/*
	 * types: 0: F(X,Y,Z) = XY v not(X) Z 1: G(X,Y,Z) = XZ v Y not(Z) 2: H(X,Y,Z) = X xor Y xor Z 3: I(X,Y,Z) = Y xor (X v not(Z)) Let [abcd k s i] denote the operation a = b + ((a + F(b,c,d) + X[k] +
	 * T[i]) <<< s).
	 */
