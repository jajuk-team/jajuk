...
public class StatView extends ViewAdapter {
  ...
  private ChartPanel createCollectionSize() {
    try {
      ...
      // contains size ( in Go ) for each month, first cell is before data
      ...
      for (int i = 0; i < iMonthsNumber + 1; i++) {
        cdata.addValue((double) lSizeByMonth[i] / 1073741824, "", getMonthsLabels(iMonthsNumber)[i]);
      }
      ...
      // tooltips
      JFreeChart jfchart = createBarChart2D(
              Messages.getString("StatView.7"), // chart title
   ...
   */
  public static JFreeChart createBarChart2D(String title, String categoryAxisLabel,
    ...
    if (tooltips) {
      renderer.setDefaultToolTipGenerator(new StandardCategoryToolTipGenerator(format, NumberFormat
              .getInstance()));
    }
    if (urls) {
  ...
  private ChartPanel createTrackNumber() {
    try {
      final DateFormat additionFormatter = UtilString.getAdditionDateFormatter();
      //      CategoryDataset cdata = null;
      // number of months we show, mounts before are set together in 'before'
      int iMonthsNumber = 5;
      // contains number of tracks for each month, first cell is 'before' data
      ...
      DefaultCategoryDataset cdata = new DefaultCategoryDataset();
      for (int i = 0; i < iMonthsNumber + 1; i++) {
        cdata.addValue(iTracksByMonth[i], "", getMonthsLabels(iMonthsNumber)[i]);
      }
      //cdata = DatasetUtilities.createCategoryDataset(new String[]{""},
      //        getMonthsLabels(iMonthsNumber), data);

      // chart, use local copy of method to use better format string for tooltips
      JFreeChart jfchart = createBarChart2D(
              Messages.getString("StatView.12"), // chart title
